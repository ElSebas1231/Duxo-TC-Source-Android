...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(FlxG.keys.justPressed.SPACE #if android || _virtualpad.buttonA.justPressed #end && UI_mainbox.selected_tab_id == 'Personaje') {
			...
			if(UI_mainbox.selected_tab_id != lastTab) {
				if(UI_mainbox.selected_tab_id == 'Animaciones') {
					...
					character.playAnim(character.jsonFile.animations[curAnim].anim);
					animText.text = 'Animaciones: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Press W or S to scroll';
				}
				lastTab = UI_mainbox.selected_tab_id;
				currentGhosts = 0;
			}
