...
class DialogueEditorState extends MusicBeatState
{
	...
	function reloadCharacter() {
		...
		if(character.animation.curAnim != null && character.jsonFile.animations != null) {
			animText.text = 'Animación: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Press W or S to scroll';
		} else {
		...
		if(!blockInput) {
			...
			for (i in 0...controlAnim.length) {
				if(controlAnim[i] && character.jsonFile.animations.length > 0) {
					...
					animText.text = 'Animación: ' + animToPlay + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Presiona W o S para ir desplazando anims.';
				}
				if(controlText[i]) {
	...
	function changeText(add:Int = 0) {
		...
		selectedText.text = 'Línea: (' + (curSelected + 1) + ' / ' + dialogueFile.dialogue.length + ') - Presiona W o S para ir desplazando.';
	}
